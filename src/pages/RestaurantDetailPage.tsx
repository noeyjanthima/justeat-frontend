...
export default function RestaurantDetailPage() {
  ...
  const { sections, items, loading, error } = useMenu(restaurantId);
  const { addItem } = useCart();
  const [active, setActive] = useState<string>("");
  const refs = useRef<Record<string, HTMLDivElement | null>>({});

  useEffect(() => {
    if (!active && sections.length) setActive(sections[0].id);
  }, [sections, active]);
  ...
  if (!restaurantId) return <div>ไม่พบร้าน</div>;
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto", padding: 16 }}>
      ...
      {/* Sections */}
      {sections.map(s => (
        <div key={s.id} ref={(el) => { refs.current[s.id] = el; }} style={{ marginTop: 24 }}>
          <h3>{s.name}</h3>
